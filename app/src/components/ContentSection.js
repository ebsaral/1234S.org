      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div>
          ...
          <div className="text-center mb-16">
            ...
            {/* Description */}
            <p className="text-justify text-lg sm:text-xl text-gray-600 leading-relaxed max-w-5xl mx-auto whitespace-pre-line">
              {t(`${sectionKey}Description`).split('**').map((part, index) => 
                    index % 2 === 1 ? <strong key={index} className="text-bold">{part}</strong> : part
                  ).map((part, index) => 
                    typeof part === 'string' && part.includes('*') 
                      ? part.split('*').map((subpart, subindex) => 
          ...
          {sectionKey === 'health' && (
            <div className="space-y-8">
              ...
              <p className="text-justify text-lg sm:text-xl text-gray-600 leading-relaxed max-w-5xl mx-auto whitespace-pre-line">
                {t(`${sectionKey}Reasoning`).split('**').map((part, index) => 
                      index % 2 === 1 ? <strong key={index} className="text-bold">{part}</strong> : part
                    ).map((part, index) => 
                      typeof part === 'string' && part.includes('*') 
                        ? part.split('*').map((subpart, subindex) => 
